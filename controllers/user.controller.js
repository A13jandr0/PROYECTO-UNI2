...
const getProfile = async (req, res) => {
  try {
    ...
    // NUEVO - Codificar banner si existe
    let bannerDataUrl = null;
    if (mongoUser.banner && mongoUser.banner.length) {
      const mime = 'image/jpeg';
      bannerDataUrl = `data:${mime};base64,${mongoUser.banner.toString('base64')}`;
    }

    // SQL extra, igual que antes
  ...
  try {
    ...
    for (const key of Object.keys(body)) {
        if (Array.isArray(body[key])) {
            // elige el primer elemento no vacío o, en su defecto, el primero
            const nonEmpty = body[key].find(v => typeof v === 'string' && v.trim() !== '');
            body[key] = nonEmpty != null ? nonEmpty : body[key][0];
        }
    }

    console.log('--- req.body:', req.body);
  console.log('--- req.files:', Object.keys(req.files || {}).reduce((o,k)=>{o[k]=req.files[k].length;return o},{}));
    ...
    if (req.files) {
      ...
      // NUEVO - Banner (solo para empresas)
      if (req.files.banner && req.files.banner[0]) {
        ...
        if (user.tipo_usuario === 'Empresa') {
          ...
          return res.status(403).json({
            success: false,
            error: 'Solo las empresas pueden tener banner'
          });
        }
      }
    }
    ...
    const soloFotos = (req.files.avatar || req.files.banner) 
               && !req.body.nombre_usuario 
               && !req.body.correo
               // además asegúrate de que no vengan campos de empresa/ONG:
               && !req.body.nombre_empresa && !req.body.NIT
               && !req.body.nombre_ong;

    if (soloFotos) {
    return res.json({
        success: true,
        message: 'Avatar/banner actualizado correctamente',
        user: mongoUser
    });
    }
