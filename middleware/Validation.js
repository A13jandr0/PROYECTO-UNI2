...
const sanitizeInput = (req, res, next) => {
    ...
    const fieldsToSanitize = [
        ...
        'titulo', 'ubicacion', 'locacion', 'motivo', 'comentarios'
    ];
...
// Validación para registro de participante externo en mega eventos (con parámetro megaEventoId)
const validateExternalParticipant = [
  ...
  body('integranteId')
    ...
    .isInt({ min: 1 })
    .withMessage('ID de integrante inválido'),
  
  body('tipoParticipacion')
    .notEmpty()
    .withMessage('El tipo de participación es requerido')
    .isIn(['participante', 'voluntario', 'ponente', 'facilitador'])
    .withMessage('Tipo de participación no válido'),
  
  handleValidationErrors
];

// Validación para patrocinio de empresa en mega eventos (con parámetro megaEventoId)
const validateCompanySponsorship = [
  ...
  body('tipoPatrocinio')
    ...
    .isIn(['patrocinador', 'auspiciador'])
    .withMessage('Tipo de patrocinio debe ser "patrocinador" o "auspiciador"'),
...
// Validación para consultas con paginación
...
// Validación de IDs de MongoDB en parámetros
...
// Validación de IDs SQL en parámetros
const validateSqlId = (paramName) => [
  param(paramName)
    .isInt({ min: 1 })
    .withMessage(`${paramName} debe ser un ID numérico válido`),
...
module.exports = {
    ...
    validateCreateMegaEvent: validateMegaEvent,
